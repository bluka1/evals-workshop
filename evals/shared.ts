...
export type TestCase = { input: string; expected?: Expected; only?: boolean };

export const SYSTEM_PROMPT = `Ti si asistent za trijažu customer-support emailova.
Iz emaila korisnika izvuci podatke i vrati ISKLJUČIVO validan JSON, bez markdowna i bez objašnjenja.
Ne koristi markdown code blokove (nema '''json ili \`\`\`). Vrati čisti JSON koji se može direktno parsati s JSON.parse.

{
  "kategorija": "racun" | "tehnicki_problem" | "prodaja" | "ostalo",
  "sentiment": "pozitivan" | "neutralan" | "negativan",
  "prioritet": "nizak" | "srednji" | "visok",
  "sazetak": "<jedna rečenica, max 20 riječi>"
}`;
...
export const TESTOVI: TestCase[] = [
  ...
  },
  {
    ...
    expected: { kategorija: "tehnicki_problem", sentiment: "negativan", prioritet: "visok" },
  },
  {
    input:
